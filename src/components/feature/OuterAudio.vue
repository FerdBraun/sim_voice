...
const gatherData = async () => {
  ...
  audioSettings.value = phrases.map(phrase => {
        ...
        return {
          // по умолчанию 0
          minutes: phrase.start_minute || 0,
          seconds: phrase.start_second || 0, // ← добавлено!
          phrase: phrase.text || '',
...
const sendPhrase = async (phrase) => {
  const res = await ApiHelper.post('global_phrases/',{at_minute: phrase.minutes,at_second: phrase.seconds,text:phrase.phrase})
  return res.file_id
}
const deletePhrase = async (id) => {
  const res = await ApiHelper.remove('global_phrases',id)
}
</script>
...
<template>
      <div class="card">
        ...
        <div class="card-header">
          <h2 class="title">Настройка звукового сопровождения вне станции</h2>
        ...
        <div class="audio-section">
          <button @click="addAudioSetting" class="add-btn">
            <span>+</span> Добавить настройки озвучивания вне станции
          </button>
